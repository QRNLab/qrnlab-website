...
| PI = admin-assigned team category | `pi` was already a supported `member_profiles.category`; the gap was the promotion path. Admin category dropdown + `POST /admin/users/:id/category` now accept `pi`. `POST /members` preserves an existing `pi` category across profile edits (users can toggle member/alumni but can't self-promote or accidentally demote a PI). Added an optional `title` field to the profile form/schema so the PI page's headline (e.g. professorship) is settable. Verified end-to-end: register → submit → approve → promote → category persists on re-submit. |
