...
/**
 * Queued static-site rebuilds. Content that is publicly visible lives in D1
 * and is exposed via `POST /api/__build`; the public site is rebuilt only
 * when an admin manually triggers a rebuild (`POST /api/rebuild`). Rows are
 * enqueued whenever a content mutation changes what is publicly visible
 * (approve/publish/edit/delete of published content). One deploy-hook POST
 * flushes the whole queue.
 */
export const pendingRebuilds = sqliteTable('pending_rebuilds', {
  id: text('id').primaryKey(),
  kind: text('kind').notNull(), // 'team' | 'blog' | 'publications' | 'updates' | 'education'
  ref: text('ref'), // entity slug/id where known
  label: text('label').notNull(), // human-readable summary for the admin queue
  createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().defaultNow(),
  status: text('status').notNull().default('pending'), // 'pending' | 'built'
  builtAt: integer('builtAt', { mode: 'timestamp_ms' }),
});
