...
import type { PublicationAuthor, BuildEntry, BuildPayload } from '@qrnlab/shared';
import { autoExcerpt, rewriteMediaUrls } from '@qrnlab/shared';
  ...
  return {
    ...
    load: async ({ store, logger, renderMarkdown, parseData }) => {
      ...
      for (const entry of entries) {
        const entryData = rewriteEntryMediaUrls(entry);
        const data = await parseData({ id: entry.id, data: entryData.data });
        let rendered;
        if (collection === 'blog' && entryData.body) {
          const r = await renderMarkdown(entryData.body);
...
/**
 * Rewrite legacy relative `/media/uploads/...` references to the absolute
 * `media.qrnlab.org` URL for every media-bearing field, regardless of which
 * data path produced the entry (local D1 or the app's `__build` endpoint).
 */
function rewriteEntryMediaUrls(entry: BuildEntry): BuildEntry {
  const data = { ...entry.data };
  if (typeof data.image === 'string') data.image = rewriteMediaUrls(data.image);
  if (Array.isArray(data.images)) data.images = (data.images as string[]).map((s) => rewriteMediaUrls(s));
  if (typeof data.description === 'string') data.description = rewriteMediaUrls(data.description);
  const body = entry.body ? rewriteMediaUrls(entry.body) : entry.body;
...
function findLocalD1File(): string | undefined {
  // D1 is owned by the admin app; its wrangler-local state is the source for
  // local site dev. Prefer the admin's state, then this package's own.
  const candidates = [
    join('..', 'admin', '.wrangler', 'state', 'v3', 'd1', 'miniflare-D1DatabaseObject'),
    join('.wrangler', 'state', 'v3', 'd1', 'miniflare-D1DatabaseObject'),
  ];
